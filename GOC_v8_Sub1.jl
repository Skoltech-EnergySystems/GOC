...
@constraint(PSCOPF, AGCLower[i in Pg[:,1]], (q[i,2] - Qmin[i])*(V[i,2] - V[i,1]) <= 0);
@constraint(PSCOPF, AGCUpper[i in Pg[:,1]], (q[i,2] - Qmax[i])*(V[i,2] - V[i,1]) <= 0);
total_part_factor = sum(part_factor);
# Post-contingency real power shortfall
@expression(PSCOPF, Delta, sum(p[i,2] for i in Pg[:,1]) - sum(p[i,1] for i in Pg[:,1]))
@constraint(PSCOPF, AGC[i in Pg[:,1]], p[i,2] == p[i,1]+(part_factor[i]/total_part_factor).*Delta);
...
open("solution1.txt", "w") do f1
    ...
    writedlm(f1, "bus id,unit id,pg(MW),qg(MVar)\r\n")
    for i in 1:sizePg
