...
function buildMod(fData,uData, contDList, contingency)
    ...
    Dictcost = collect(gData[k].cParams for k in collect(keys(gData)))
    c = collect(Dictcost[k][:0] for k in collect(keys(Dictcost)))
    # Linear term b in cost function
    b = collect(Dictcost[k][:1] for k in collect(keys(Dictcost)))
    # Quadratic term a in cost function
    a = collect(Dictcost[k][:2] for k in collect(keys(Dictcost)))
    # Participation factor of generator
    part_factor = collect(gData[k].alpha for k in collect(keys(gData)))
    ...
    for i = 1:size(bList,1)
        for j = 1:size(brList,1)
            if collect(bData[k].ID_ext for k in collect(keys(bData)))[i] == collect(brData[k].ID_ext for k in collect(keys(brData)))[j][1]
                br_int[j,1] = collect(bData[k].ID_int for k in collect(keys(bData)))[i]
            end
            if collect(bData[k].ID_ext for k in collect(keys(bData)))[i] == collect(brData[k].ID_ext for k in collect(keys(brData)))[j][2]
                br_int[j,2] = collect(bData[k].ID_int for k in collect(keys(bData)))[i]
            end
        end
    end
    #br_int
    ...
    # series admittance
    Ys = 1 ./ (collect(brData[k].r for k in collect(keys(brData))) + im * collect(brData[k].x for k in collect(keys(brData))))
    # line charging susceptance
    Bc = collect(brData[k].bc for k in collect(keys(brData)))
    # transformer phase shirters
    shift = zeros(size(brList,1),1)
    # tap ratios
    tap = collect(brData[k].tau for k in collect(keys(brData))).*exp.(im*pi/180*shift)
    ...
    Ytf = - Ys ./ tap;
    # Shunt admittance
    Ysh = (collect(bData[k].gsh for k in collect(keys(bData))) + im * collect(bData[k].bsh for k in collect(keys(bData))))
    ...
    BL = imag.(Ybus);
    # Generator limits
    Pmax = collect(gData[k].Pmax for k in collect(keys(gData)))
    Pmin = collect(gData[k].Pmin for k in collect(keys(gData)))
    Qmax = collect(gData[k].Qmax for k in collect(keys(gData)))
    Qmin = collect(gData[k].Qmin for k in collect(keys(gData)))
    # Branch limits (MVA)
    RATE_A = collect(brData[k].t for k in collect(keys(brData)))
    # Voltage limits
    Vmax = collect(bData[k].Vmax for k in collect(keys(bData)))
    Vmin = collect(bData[k].Vmin for k in collect(keys(bData)))
    # Demand
    Pd = collect(bData[k].Pd for k in collect(keys(bData)))
    Qd = collect(bData[k].Qd for k in collect(keys(bData)))
    ...
    # Contingency matrix
    Br_ind = hcat(collect(1:NBr),collect(brData[k].From for k in collect(keys(brData))),collect(brData[k].To for k in collect(keys(brData))));
    ...
    for k=1:NBr
      links[k] = (collect(brData[k].From for k in collect(keys(brData)))[k,1], collect(brData[k].To for k in collect(keys(brData)))[k,1])   # line from-to (directed)
    end
    ...
    collect(gData[k].ID_ext for k in collect(keys(gData)))
    collect(bData[k].ID_ext for k in collect(keys(bData)))
    # "generators-buses" incidence matrix
    Ω = zeros(NBus,NGen);

    for j = 1:NGen
        for i = 1:NBus
            if collect(gData[k].ID_ext for k in collect(keys(gData)))[j] == collect(bData[k].ID_ext for k in collect(keys(bData)))[i] &&
                 j in collect(gData[k].ID_int for k in collect(keys(gData))) &&
                 i in collect(bData[k].ID_int for k in collect(keys(bData)))
                Ω[i,j] = 1
            end
        end
    end
