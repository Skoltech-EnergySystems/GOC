...
@constraint(PSCOPF, EqConstrP[i=1:NBus,k=1:NK], sum(Ω[i,g]*p[g,k] for g=1:NGen) - Pd[i] == GL[i,i]*c[i,i,k] + sum(Ψ[i,j]*GL[i,j]*c[i,j,k] - Ψ[i,j]*BL[i,j]*s[i,j,k] for j=1:NBus));
# Nodal reactive power balance
@constraint(PSCOPF, EqConstrQ[i=1:NBus,k=1:NK], sum(Ω[i,g]*q[g,k] for g=1:NGen) - Qd[i] == -BL[i,i]*c[i,i,k] + sum(-Ψ[i,j]*BL[i,j]*c[i,j,k] - Ψ[i,j]*GL[i,j]*s[i,j,k] for j=1:NBus));
...
# SOCP relaxation
@constraint(PSCOPF, SOCP[i=1:NBus, j=1:NBus, k=1:NK], c[i,j,k]^2 + s[i,j,k]^2 + (0.5*(c[i,i,k]-c[j,j,k]))^2 ≤ (0.5*(c[i,i,k]+c[j,j,k]))^2);
# Vm => 0.9
...
# Deriving line active power flows
getvalue(pl)

# Deriving line reactive power flows
getvalue(ql)
